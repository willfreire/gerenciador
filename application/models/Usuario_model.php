...
class Usuario_model extends CI_Model {
     ...
     *
     * @method alterUsuario
     * @param obj $valores Dados para edição
     * @access public
     * @return obj Status de ação
     */
    public function alterUsuario($valores)
    {
        # Atribuir vars
        $retorno = new stdClass();
        $dados   = array();
        ...
        $dados['nome']  = $valores->nome;
        $dados['login'] = $valores->login;
        if ($valores->senha != NULL):
            $dados['senha'] = $valores->senha;
        endif;
        $dados['id_perfil_fk'] = $valores->perfil;
        $dados['id_status_fk'] = $valores->status;

        # Grava usuario
        $this->db->where('id_deops_user_pk', $valores->id);
        $this->db->update('tb_deops_user', $dados);
