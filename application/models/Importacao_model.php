...
class Importacao_model extends CI_Model {
    ...
    /**
     * Método responsável por pesquisar e buscar Importacaos
     *
     * @method getImportacaos
     * @param obj $search Conjuntos de dados para realizar a pesquisa
     * @access public
     * @return obj Lista de beneficios
     */
    public function getImportacaos($search)
    {
        # Atribuir valores
        ...
        if (!empty($resp_dados)):
            ...
            foreach ($resp_dados as $value):
                ...
                $acao       = "<button type='button' class='btn btn-success btn-xs btn-acao' title='Editar Benef&iacute;cio' onclick='Importacao.redirect(\"$url_edit\")'><i class='glyphicon glyphicon-edit' aria-hidden='true'></i></button>";
                $acao      .= "<button type='button' class='btn btn-primary btn-xs btn-acao' title='Visualizar Benef&iacute;cio' onclick='Importacao.redirect(\"$url_view\")'><i class='glyphicon glyphicon-eye-open' aria-hidden='true'></i></button>";
                if ($this->session->userdata('id_perfil_vt') == "1"):
                    $acao.= "<button type='button' class='btn btn-danger btn-xs btn-acao' title='Excluir Benef&iacute;cio' onclick='Importacao.del(\"$id_benef\")'><i class='glyphicon glyphicon-remove' aria-hidden='true'></i></button>";
                endif;
    ...
    /**
     * Método de exclusão de um Importacao
     *
     * @method delImportacao
     * @access public
     * @param integer $id Id do registro a ser excluído
     * @return obj Status da ação
     */
    public function delImportacao($id)
    {
        # Atribuir vars
        $retorno = new stdClass();
